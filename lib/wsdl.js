...
WSDL.prototype.findChildSchemaObject = function(parameterTypeObj, childName, count) {
  ...
  if (object.$type) {
    ...
    if (typeDef) {
      return this.findChildSchemaObject(typeDef, childName, count+1);
    }
  }

  if (object.children) {
    for (i = 0, child; child = object.children[i]; i++) {
      found = this.findChildSchemaObject(child, childName, count+1);
      if (found) {
        break;
      }
      ...
      if (child.$base) {
        ...
        if (foundBase) {
          found = this.findChildSchemaObject(foundBase, childName, count+1);
